...
def main(data_dir, chk_pt, logger_file=None):
  global logger_h
  logger_h = create_logger(log_level, logger_file)
  synth_data = load_data(data_dir)
  plot_data = logger_h.getEffectiveLevel() <= logging.DEBUG 
  if plot_data:
    plot_label_dist(synth_data)
  synth_dataset = OpeNPDNDataset(synth_data, 
                                labels_present=True,
                                normalize=True)                              
  set_global_hyperparams()
  trained_model = train_loop(synth_dataset, plot_data)
  torch.save(trained_model, chk_pt) 
  synth_dataset.store_normalize(os.path.dirname(chk_pt))
  if plot_data:
    plt.show()
...
def train_loop(dataset, plot=False): 
  batch_size =1
  ...
  while epoch<epochs:
    for step, data in enumerate(train_loader):
      ...
      with torch.no_grad():
        ...
        inner_loop.set_postfix({'Train_Acc':"%06.3f"%(100*(running_correct/running_total)),
                                'Loss':"%10.5f"%(running_train_loss/(step+1))},refresh=False)
        inner_loop.update(1)
        
        if(step%len(train_loader) == len(train_loader)-1):
          train_loss_results.append(running_train_loss/(step+1))
          train_accuracy_results.append(100*(running_correct/running_total))
          valid_total = 0.0
          valid_correct = 0.0
          val_loss = 0
          ...
          valid_accuracy_results.append(100*(valid_correct/valid_total))
...
def load_data(data_dir):
  ...
  input_data = []
  for n,filename in enumerate(tqdm(glob("%s/synth_data_*.npz"%data_dir))):
    input_data.append(np.load(filename))
...
def plot_label_dist(input_data):
  input_labels = []
  num_data_pts = len(input_data)
  for num, data_pt in enumerate(reversed(input_data)):
    idx = num_data_pts -1 - num
...
if __name__ == '__main__':
  ...
  data_dir = "./run/data"
  chk_pt = "./run/checkpoint/synth_CNN.ckp"
  main(data_dir,chk_pt) 
